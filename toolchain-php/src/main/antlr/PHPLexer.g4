...
// based on https://github.com/antlr/grammars-v4/blob/dc41ab3bf14031de2b9a90230298895a60b13e20/php/PHPLexer.g4
lexer grammar PHPLexer;

// alexsaveliev: added package and import
@header {
    package com.sourcegraph.toolchain.php.antlr4;
...
@lexer::members
{
// alexsaveliev: replaced C# code with java equivalent
	boolean ScriptTag;
	boolean StyleTag;
	...
	public Token nextToken() {
		...
		if (_mode == HereDoc) {
			...
			switch (token.getType()) {
				case StartHereDoc:
				case StartNowDoc:
					HeredocIdentifier = StringUtils.strip(token.getText().substring(3).trim(), "'");
	...
	boolean CheckHeredocEnd(String text) {
		...
		return HeredocIdentifier.equals(identifier);
	}
}
